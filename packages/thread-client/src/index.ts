...
export class ThreadClient {
  ...
  create(options: CreateThreadOptions = {}): Promise<AgentThread> {
    return this.request("/v2/threads", {
      method: "POST",
      body: JSON.stringify({ requestId: options.requestId ?? requestId(), agentId: options.agentId }),
  ...
  async list(options: ListThreadsOptions = {}): Promise<ThreadPage> {
    ...
    return threadPageSchema.parse(await this.request(`/v2/threads${query.size ? `?${query}` : ""}`));
  }

  get(threadId: string): Promise<AgentThread> {
    return this.request(`/v2/threads/${encodeURIComponent(threadId)}`).then((value) => threadSchema.parse(value));
  }
  ...
  async messages(
    ...
    return threadMessagePageSchema.parse(await this.request(
      `/v2/threads/${encodeURIComponent(threadId)}/messages${query.size ? `?${query}` : ""}`,
    ));
  }

  rename(threadId: string, title: string): Promise<AgentThread> {
    return this.request(`/v2/threads/${encodeURIComponent(threadId)}`, {
      method: "PATCH",
  ...
  async delete(threadId: string): Promise<void> {
    await this.request(`/v2/threads/${encodeURIComponent(threadId)}`, { method: "DELETE" });
  }
    ...
    void (async () => {
      ...
      while (!controller.signal.aborted) {
        try {
          ...
          const response = await this.requestFetch(`${this.baseUrl}/v2/thread-events`, {
            headers,
  ...
  private setStatus(threadId: string, action: "archive" | "unarchive"): Promise<AgentThread> {
    return this.request(`/v2/threads/${encodeURIComponent(threadId)}/${action}`, { method: "POST" })
      .then((value) => threadSchema.parse(value));
  }
